...
module.exports = function (Artwork) {
  ...
  Artwork.upload = function (title, width, height, data, cb) {
    var fileName = uuid.v1();
    var container = 'common';
    var model = {
      uuid: fileName,
    ...
    var streamOut = Artwork.app.models.Container.uploadStream(container, fileName + '.png', options);
    ...
    streamOut.once(
      ...
      function (err, fileObj) {
        if (err) {
          ...
          console.log(`Succefully wrote file output for ${JSON.stringify(model)}.  Options were ${JSON.stringify(options)}.`);
    ...
    let failCount = 0;
    let currentIndex = 0;
    let nextIndex = Math.min(dataSize, writeUnit);
    let writeMessage;
    ...
    function doSomeWrites() {
      ...
      if (currentIndex < dataSize) {
        if (failCount >= maxRepeatFail) {
          let errorMessage =
            `Failed to write data body of size ${data.length} for ${JSON.stringify(model)}.  Options were ${JSON.stringify(options)}.`;
  ...
  Artwork.remoteMethod(
    'upload',
    {
      ...
      accepts: [
        ...
        {arg: 'width', type: 'string', http: {source: 'form'}},
        {arg: 'height', type: 'string', http: {source: 'form'}},
